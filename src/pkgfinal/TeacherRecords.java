...
public class TeacherRecords extends javax.swing.JFrame {
    ...
    public TeacherRecords() {
        initComponents();
        teacherDatabase = TeacherDatabase.getInstance();
    ...
    if (tid.isEmpty()) {
        ...
        // Use TeacherDatabase to get teacher record
        Teachers teacher = teacherDatabase.getTeacherById(tid);

        if (teacher != null) {
            // Populate UI fields with teacher information
            Name.setText(teacher.getName());
            Cno.setText(teacher.getContactNumber());
            Add.setText(teacher.getAddress());
            Sal.setText(teacher.getSalary());
            Dept.setSelectedItem(teacher.getDepartment());
            
            // Extracting date components from DOB
            String[] dobParts = teacher.getDob().split("/");
            ddob.setSelectedItem(dobParts[2]); // Day
            mdob.setSelectedItem(getMonthName(Integer.parseInt(dobParts[1]))); // Month
            ydob.setSelectedItem(dobParts[0]); // Year

            Post.setSelectedItem(teacher.getPosition());
...
private String getMonthName(int month) {
    ...
    if (tid.isEmpty()) {
        ...
        String dd = (String) ddob.getSelectedItem();
        String mm = String.valueOf(mdob.getSelectedIndex() + 1);
        String yy = (String) ydob.getSelectedItem();
        String dob = yy + "/" + mm + "/" + dd;
        ...
        if (nm.isEmpty() || cno.isEmpty() || add.isEmpty() || sal.isEmpty()) {
            ...
            try {
                // Use TeacherDatabase to update teacher record
                Teachers updatedTeacher = new Teachers(tid, nm, dept, dob, cno, add, post, sal);
                teacherDatabase.updateTeacher(updatedTeacher);
    ...
    if (tid.isEmpty()) {
        ...
        try {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                // Use TeacherDatabase to delete teacher record
                teacherDatabase.deleteTeacher(tid);
